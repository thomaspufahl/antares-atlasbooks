USE AtlasBooks
GO

--- START INSERT
IF OBJECT_ID('abInsLoan') IS NOT NULL
	DROP PROC abInsLoan
GO

CREATE PROCEDURE abInsLoan
	@editorUserId int,
	@returnDate   datetime,
	@userId       int,
	@bookId       int
AS
BEGIN
	IF (dbo.isAdmin(@editorUserId) = 0) RETURN;
	IF EXISTS(SELECT 1 FROM [User] WHERE userId = @userId AND isActive = 0) RETURN;
	IF EXISTS(SELECT 1 FROM [Book] WHERE bookId = @bookId AND isActive = 0) RETURN;

	SET DATEFORMAT ymd;

	INSERT INTO [Loan] 
	(returnDate, userId, bookId)
	VALUES
	(@returnDate, @userId, @bookId)

END;
GO
--- END INSERT




--- START UPDATE
IF OBJECT_ID('abUpdLoan') IS NOT NULL
	DROP PROC abUpdLoan
GO

CREATE PROC abUpdLoan
	@editorUserId int,
	@loanId       int,
	@returnDate   datetime
AS
BEGIN
	IF EXISTS(SELECT 1 FROM [Loan] WHERE loanId = @loanId AND isActive = 0) RETURN;
	IF (dbo.isAdmin(@editorUserId) = 0) RETURN;

	UPDATE [Loan] 
	SET
	returnDate = @returnDate
	WHERE loanId = @loanId

END;
GO
--- END UPDATE



--- START DELETE
IF OBJECT_ID('abDelLoan') IS NOT NULL
	DROP PROC abDelLoan
GO

CREATE PROC abDelLoan
	@editorUserId int,
	@loanId       int
AS
BEGIN
	IF EXISTS(SELECT 1 FROM [Loan] WHERE loanId = @loanId AND isActive = 0) RETURN;
	IF (dbo.isAdmin(@editorUserId) = 0) RETURN;
	

	UPDATE [Loan] SET isActive = 0 WHERE loanId = @loanId

END;
GO
--- END DELETE


--- START GET
IF OBJECT_ID('abGetLoan') IS NOT NULL
	DROP PROC abGetLoan
GO

CREATE PROC abGetLoan
	@editorUserId int,
	@getAll       bit = 0
AS
BEGIN
	IF (dbo.isAdmin(@editorUserId) = 1 AND @getAll = 1)
		BEGIN
			SELECT * FROM Loan
			RETURN;			
		END

	SELECT * FROM Loan WHERE isActive <> 0
END
GO
--- END GET